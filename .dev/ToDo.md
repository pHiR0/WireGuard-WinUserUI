...
+ Directamente en la APP veo en la seccion Auditoría, que se generan muchos eventos, sobre todo de tipo ListTunnels, y lo que temo es que esto pueda generar un archivo gigante donde se almacenen las auditorías
> Para ello me gustaría en la configuración , en la seccion de Cofiguración global (Administradores) añadir un apartado específico para las auditorías, donde :
> Poder desactivarla globalmente, esto quiere decir que no aparece la pestaña pero tampoco se guardan los eventos para auditar. Esta opcion debe almacenarse en el registro de windows, por defecto vendrá activada.
> Limitar el tamaño del archivo donde se almacenan las auditorías; especificado en KB o MB (ambas opciones) y una vez llegado tiene que ejecutarse una limpieza automática que elimine los eventos mas antiguos para quedar justo un 10% por debajo del límite.
> Tambien si cuando se vaya regsitrar un evento, si resulta que es del mismo usuario accion y resultado que el ultimo, estaría bien agruparlos, y podemos reflejar fecha de inicio (desde) hasta fecha fin (ultimo evento) y tambien un contador de veces. No solo quiero agruparlos en la UI tambien a la hora de almacenarlos
# Implementado: GlobalSettingsStore con GetAuditEnabled/SetAuditEnabled (HKLM registry, default true) y GetAuditMaxSizeKb/SetAuditMaxSizeKb. JsonAuditLogger reescrito: (1) Deduplicación — detecta último evento en fichero al arrancar, si el nuevo evento tiene mismo Username+Action+Result+Tunnel actualiza TimestampEnd y Count truncando y reescribiendo la última línea in-place sin leer el fichero entero. (2) Trimado por tamaño — cada 10 escrituras comprueba el tamaño; si supera el límite elimina entradas antiguas hasta quedar al 90%. (3) Check enabled — si AuditEnabled=false la llamada a LogAsync retorna inmediatamente. AuditEntryDto añade TimestampEnd y Count con helpers IsGrouped y CountText para la UI. IPC: nuevos comandos GetAuditSettings/SetAuditSettings (Admin). UI: pestaña Auditoría IsVisible=IsAuditTabVisible (IsAdmin && AuditEnabled). Settings tab: bloque admin de auditoría con toggle enable y NumericUpDown+ComboBox para límite en KB/MB. Entradas agrupadas muestran rango de fechas y badge ×N.


+ Quiero que el valor por defecto de AuditMaxSizeKb sea 1024KB (su equivalente para almacenar correctamente en el registro) y que tenga que se un valor superior a 0, no quiero que permita ilimitado.
> Si por lo que sea no existe la clave o es 0 o inferior se reconfigura a valor por defecto
> La casilla de texto en la configuración donde ponemos el numero con el tamaño máximo, hazla un poco mas grande horizontalmente , que quepan unos 3 caracteres mas.
# Implementado: GlobalSettingsStore.GetAuditMaxSizeKb: default 1024, si el valor en registry es <=0 se escribe 1024 automáticamente. SetAuditMaxSizeKb: Math.Max(1, sizeKb) para garantizar mínimo 1 KB. ViewModel: si el servicio devuelve 0, se muestra 1 MB. ComputeAuditMaxSizeKb: Math.Max(1, kb). AXAML: Minimum="1" en NumericUpDown, Width 110→140, eliminado "(0 = sin límite)" del label, hint text actualizado con valores por defecto.